...
int CNvEncoder::EncodeMain(int argc, char *argv[])
{
    ...
    {
        ifstream file(encodeConfig.qpDeltaMapFile, ios::in|ios::binary|ios::ate);
        if (file.is_open()) {
            size = file.tellg();
            buffer = new char [size];
            qpDeltaMap = new int8_t [size];
            file.seekg(0, ios::beg);
            file.read(buffer, size);
            file.close();
            for (int i=0; i<size; i++) {
                 qpDeltaMap[i] = int8_t(buffer[i]);
            }
            qpDeltaMapSize = size;
        } else {
            cout << "failed to load qpDeltaMapFile" << endl;
            encodeConfig.qpDeltaMapFile = NULL;
        }
    }
    ...
    if (encodeConfig.qpDeltaMapFile)
    {
        nvStatus = EncodeFrame(NULL, true, encodeConfig.width, encodeConfig.height, qpDeltaMap, qpDeltaMapSize);
    }
    else
    {
...
NVENCSTATUS CNvEncoder::EncodeFrame(EncodeFrameConfig *pEncodeFrame, bool bFlush, uint32_t width, uint32_t height, int8_t *qpDeltaMap, uint32_t qpDeltaMapSize)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
    uint32_t lockedPitch = 0;
    ...
    nvStatus = m_pNvHWEncoder->NvEncEncodeFrame(pEncodeBuffer, NULL, width, height, (NV_ENC_PIC_STRUCT)m_uPicStruct, qpDeltaMap, qpDeltaMapSize);
    return nvStatus;
}
